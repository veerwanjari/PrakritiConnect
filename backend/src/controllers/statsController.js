...
export const leaderboard = async (req, res) => {
  try {
    const top = await User.find({ role: { $in: ['volunteer', 'organizer'] }, isBlocked: false })
      .sort({ points: -1 })
      .limit(10)
      .select('name points');
...
export const summary = async (_req, res) => {
  try {
    const [totalEvents, approvedEvents, upcomingEvents, totalRegistrations, totalVolunteers, totalOrganizers] = await Promise.all([
      Event.countDocuments({}),
      Event.countDocuments({ status: 'approved' }),
      Event.countDocuments({ status: 'approved', date: { $gte: new Date() } }),
      Registration.countDocuments({}),
      User.countDocuments({ role: 'volunteer', isBlocked: false }),
